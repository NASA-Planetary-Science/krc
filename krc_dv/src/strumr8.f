      SUBROUTINE STRUMR8 (PARI,NIN,S1, SSS,LOUT)
C_Titl  STRUMR8  Concatonate integers into one string using separator
C            ^^     ^=places that change between routines in this family
      IMPLICIT NONE
C_Vars
      REAL*8  PARI(NIN)         ! in. values to transfer
C     ^^^^^^^
...
C_End
      ...
      CHARACTER(LEN=15) BUFF,BUF2 ! local strings
C                   ^^
